...
| `training/test_live_model_swap.py` | STEP 8: `SimulationManager.set_model` loads a real trained checkpoint into the live loop, flips `is_trained` honestly, reverts on `untrained`, and raises for a missing checkpoint (no fake badge). |
